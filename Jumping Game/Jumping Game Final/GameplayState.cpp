...
{
	if (m_pLevel->IsWall(newPlayerX, (newPlayerY + 1)))
	{
		m_FloorBelow = true;
	}
	else
...
void GameplayState::Draw()
{
	...
	else if (viewportLeft + kViewportWidth > m_pLevel->GetWidth()) //Keeps the viewport from leaving the array on the right
	{
		viewportLeft = m_pLevel->GetWidth() - kViewportWidth;
	}
	...
	else if (viewportTop + kViewportHeight > m_pLevel->GetHeight())
	{
		viewportTop = m_pLevel->GetHeight() - kViewportHeight;
	}

	m_pLevel->Draw(viewportLeft, viewportTop, kViewportWidth, kViewportHeight);
	...
	currentCursorPosition.Y = kViewportHeight - 1;
